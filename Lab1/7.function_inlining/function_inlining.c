...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) { //1. loop interchange
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			// 1st iteration
			int it1= (i - 1)*SIZE +j;
			res += input[it1 - 1] * horiz_operator[0][0];
			res += input[it1] * horiz_operator[0][1];
			res += input[it1 + 1] * horiz_operator[0][2];

			// 2nd iteration
			int it2 = i*SIZE + j;
			res += input[it2 - 1] * horiz_operator[1][0];
			res += input[it2] * horiz_operator[1][1];
			res += input[it2 + 1] * horiz_operator[1][2];

			// 3rd iteration
			int it3 = (i + 1)*SIZE + j;
			res += input[it3 -1] * horiz_operator[2][0];
			res += input[it3] * horiz_operator[2][1];
			res += input[it3 + 1] * horiz_operator[2][2];

			p = pow(res,2);
			res=0;

			// 1st iteration
			res += input[it1 - 1] * vert_operator[0][0];
			res += input[it1] * vert_operator[0][1];
			res += input[it1 + 1] * vert_operator[0][2];

			// 2nd iteration
			res += input[it2 - 1] * vert_operator[1][0];
			res += input[it2] * vert_operator[1][1];
			res += input[it2 + 1] * vert_operator[1][2];

			// 3rd iteration
			res += input[it3 -1] * vert_operator[2][0];
			res += input[it3] * vert_operator[2][1];
			res += input[it3 + 1] * vert_operator[2][2];
