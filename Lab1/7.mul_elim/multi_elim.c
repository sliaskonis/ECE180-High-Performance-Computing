...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) { //1. loop interchange
		...
		for (j=1; j<SIZE-1; j+=1 ) {
			 ...
			 * of the derivative.								  */
			res = 0;
			res += input[(i - 1)*SIZE + j -1] * horiz_operator[0][0];
			res += input[(i - 1)*SIZE + j] * horiz_operator[0][1];
			res += input[(i - 1)*SIZE + j + 1] * horiz_operator[0][2];

			// 2nd iteration
			res += input[(i)*SIZE + j -1] * horiz_operator[1][0];
			res += input[(i)*SIZE + j] * horiz_operator[1][1];
			res += input[(i)*SIZE + j + 1] * horiz_operator[1][2];

			// 3rd iteration
			res += input[(i + 1)*SIZE + j -1] * horiz_operator[2][0];
			res += input[(i + 1)*SIZE + j] * horiz_operator[2][1];
			res += input[(i + 1)*SIZE + j + 1] * horiz_operator[2][2];

			p = pow(res,2);
			res=0;

			// 1st iteration
			res += input[(i - 1)*SIZE + j -1] * vert_operator[0][0];
			res += input[(i - 1)*SIZE + j] * vert_operator[0][1];
			res += input[(i - 1)*SIZE + j + 1] * vert_operator[0][2];

			// 2nd iteration
			res += input[(i)*SIZE + j -1] * vert_operator[1][0];
			res += input[(i)*SIZE + j] * vert_operator[1][1];
			res += input[(i)*SIZE + j + 1] * vert_operator[1][2];

			// 3rd iteration
			res += input[(i + 1)*SIZE + j -1] * vert_operator[2][0];
			res += input[(i + 1)*SIZE + j] * vert_operator[2][1];
			res += input[(i + 1)*SIZE + j + 1] * vert_operator[2][2];
